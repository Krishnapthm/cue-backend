...
_SYSTEM_PROMPT = (
    ...
    "cap the ingredient list for the sake of brevity. Flagging an ingredient "
    "(below) is not the same as omitting it - keep the list complete and let "
    "the flags decide what the user is asked about.\n"
    "- Set pantry_staple=true on any ingredient a home cook is assumed to "
    "already have and would never add to a grocery order for this dish: "
    "water, salt, sugar, cooking oil, everyday ground spices and dried "
    "seasonings used in background amounts (turmeric, red chilli powder, "
    "dried red chillies, cumin, mustard seeds, black pepper, garam masala, "
    "asafoetida, curry powder). Judge it by the role the ingredient plays "
    "here, not by its name: set it false when the ingredient is a headline "
    "component of THIS dish or is called for in an amount someone would have "
    "to go and buy - a chilli paste built on 200 g of dried red chillies, a "
    "salt crust, a deep-fry needing a litre of oil. Everything else defaults "
    "to false; when genuinely unsure, leave it false.\n"
    "- Set user_supplied=true on any ingredient the user's own message says "
    "they already have ('I already have idli batter', 'I've got the "
    "paneer'). Read this off what they actually wrote - never infer it, "
    "never guess it from the dish, and never set it because an ingredient "
    "seems common. If they say they have a prepared component, list that "
    "component itself as one ingredient with user_supplied=true and do NOT "
    "list the things it would have been made from, since they are not making "
    "it.\n"
    ...
    "including when the user already told you they have that component, "
...
_PHOTO_SYSTEM_PROMPT = (
    ...
    "- Set pantry_staple=true on any ingredient a home cook is assumed to "
    "already have and would never add to a grocery order for this dish: "
    "water, salt, sugar, cooking oil, and everyday ground spices or dried "
    "seasonings used in background amounts. Set it false when the ingredient "
    "is a headline component of the dish, or is called for in an amount "
    "someone would have to go and buy. When unsure, leave it false. Leave "
    "user_supplied false on every ingredient: this path reads a photo, and "
    "nobody has told you what they already have.\n"
    "- Quantities and units are optional per ingredient - omit them (leave "
